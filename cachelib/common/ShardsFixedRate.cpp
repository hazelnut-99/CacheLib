...
uint64_t ShardsFixedRate::getDistance(HashedKey const& hk) {
    uint64_t distance = 0;
...
void ShardsFixedRate::feed(HashedKey hk) {
    if (hk.keyHash() % kP < kT) {
        m_objectCounter++;
        uint64_t const distance = getDistance(hk) / kR;
        updateHistogram((distance == 0) ? 0 : (((distance - 1) / kBucketSize) + 1));
    }
}

std::unordered_map<uint64_t, uint64_t> ShardsFixedRate::getReuseDistanceHistogram() const {
    return m_distanceHistogram;
}

void ShardsFixedRate::resetReuseDistanceHistogram() {
