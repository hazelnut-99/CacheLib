 ...
 */
 ...
 namespace facebook::cachelib {
    ...
    HitsPerTailSlabStrategy::HitsPerTailSlabStrategy(Config config)
     : RebalanceStrategy(HitsPerTailSlab), config_(std::move(config)) {}
 ...
 ClassId HitsPerTailSlabStrategy::pickVictim(const Config& config,
   ...
   for (auto it = victims.begin(); it != victims.end(); ++it) {
     ...
     const auto deltaHitsValue = poolState.at(id).getMarginalHits(stats, config.tailSlabCnt);
   ...
   return *std::min_element(
       victims.begin(), victims.end(), [&](ClassId a, ClassId b) {
         ...
         return poolState.at(a).getMarginalHits(stats, config.tailSlabCnt) * weight_a <
                poolState.at(b).getMarginalHits(stats, config.tailSlabCnt) * weight_b;
       });
 }
 ...
 ClassId HitsPerTailSlabStrategy::pickReceiver(const Config& config,
                                           PoolId pid,
                                           const PoolStats& stats,
 ...
 RebalanceContext HitsPerTailSlabStrategy::pickVictimAndReceiverImpl(
   ...
   const auto victimTailHitsPerSlab =
       poolState.at(ctx.victimClassId).getMarginalHits(poolStats, config.tailSlabCnt) *
       weightVictim;
   const auto receiverTailHitsPerSlab =
       poolState.at(ctx.receiverClassId).getMarginalHits(poolStats, config.tailSlabCnt) *
       weightReceiver;
 
   XLOGF(DBG,
         "Rebalancing: receiver = {}, receiver delta marginal hits per slab = {}, victim "
         "= {}, victim delta marginal hits per slab = {}",
         static_cast<int>(ctx.receiverClassId), receiverTailHitsPerSlab,
         static_cast<int>(ctx.victimClassId), victimTailHitsPerSlab);
 
   const auto improvement =
   receiverTailHitsPerSlab - victimTailHitsPerSlab;
   if (receiverTailHitsPerSlab <= victimTailHitsPerSlab ||
       improvement < config.minDiff ||
       improvement < config.diffRatio * static_cast<long double>(
        victimTailHitsPerSlab)) {
 ...
 ClassId HitsPerTailSlabStrategy::pickVictimImpl(const CacheBase& cache,
                                             PoolId pid,
