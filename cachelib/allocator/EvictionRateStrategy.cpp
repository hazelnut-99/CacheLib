...
EvictionRateStrategy::EvictionRateStrategy(Config config)
    : RebalanceStrategy(EvictionRate), config_(std::move(config)) {}
...
//
// 2. pick victim from the one that has smallest eviction rate
ClassId EvictionRateStrategy::pickVictim(const Config& config,
  ...
  for (auto it = victims.begin(); it != victims.end(); ++it) {
    ...
    const auto deltaEvictions = poolState.at(id).getDeltaAllocations(stats);
    XLOGF(DBG, "Class ID: {}, nSlab: {}, deltaEvictions: {}", static_cast<int>(id), nSlab, deltaEvictions);
  }
  ...
  return *std::min_element(
      victims.begin(), victims.end(), [&](ClassId a, ClassId b) {
        ...
        return poolState.at(a).getDeltaAllocations(stats) * weight_a <
               poolState.at(b).getDeltaAllocations(stats) * weight_b;
      });
}
...
//
// 2. pick receiver from the one that has the largest eviction rate
ClassId EvictionRateStrategy::pickReceiver(const Config& config,
                                          PoolId pid,
                                          const PoolStats& stats,
...
RebalanceContext EvictionRateStrategy::pickVictimAndReceiverImpl(
  ...
  const auto victimDeltaEvictions =
      poolState.at(ctx.victimClassId).getDeltaAllocations(poolStats) *
      weightVictim;
  const auto receiverDeltaEvictions =
      poolState.at(ctx.receiverClassId).getDeltaAllocations(poolStats) *
      weightReceiver;

  XLOGF(DBG,
        "Rebalancing: receiver = {}, receiver delta evictions = {}, victim "
        "= {}, victim delta evictions = {}",
        static_cast<int>(ctx.receiverClassId), receiverDeltaEvictions,
        static_cast<int>(ctx.victimClassId), victimDeltaEvictions);

  const auto improvement =
      receiverDeltaEvictions - victimDeltaEvictions;
  if (receiverDeltaEvictions < victimDeltaEvictions ||
      improvement < config.minDiff ||
      improvement < config.diffRatio * static_cast<long double>(
                                           victimDeltaEvictions)) {
  ...
  for (const auto i : poolStats.getClassIds()) {
    poolState[i].updateAllocations(poolStats);
  }

  return ctx;
}

ClassId EvictionRateStrategy::pickVictimImpl(const CacheBase& cache,
                                            PoolId pid,
